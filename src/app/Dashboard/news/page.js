...
export default function News() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteData(`/news/${selectedItem.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    if (!formData.Image) {
      toast("الرجاء اختيار صورة للخبر.");
      return;
    }
    ...
    try {
      await addData("/news", formDataToSend);
      setIsOpen(false);
      toast.success('تم اضافة الخبر');
      setLoading(false);
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={index} className="border-b hover:bg-gray-100 transition-all">
                ...
                <td className="px-4 py-2 text-center">{item.Title.length > 20 ? item.Title.substring(0, 20) + "..." : item.Title}</td>
                <td className="px-4 py-2 text-center">{item.Description.length > 30 ? item.Description.substring(0, 30) + "..." : item.Description}</td>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <Dialog.Panel className="bg-white p-6 rounded-lg shadow-lg w-96">
            <Dialog.Title className="text-lg font-bold">إضافة خبر جديد</Dialog.Title>
