...
export default function AnnouncementsTypes() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteData(`/announcement-types/${selectedItem.id}`);
      setIsDeleteOpen(false);
      toast.error("تم حذف العنصر بنجاح");
      getData();
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await addData("/announcement-types", formData);
      setIsOpen(false);
      setLoading(false);
      toast.success('تم اضافة نوع الاعلان');
      getData();
    } catch (error) {
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={index} className="border-b hover:bg-gray-100 transition-all">
                ...
                <td className="px-4 py-2 text-center">{item.AnnoucementType.length > 20 ? item.Title.substring(0, 20) + "..." : item.AnnoucementType}</td>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <Dialog.Panel className="bg-white p-6 rounded-lg shadow-lg w-96">
            <Dialog.Title className="text-lg font-bold">إضافة نوع اعلان جديد</Dialog.Title>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
              <input type="text" name="AnnoucementType" value={formData.Title} onChange={handleInputChange} placeholder="العنوان" className="border p-2 rounded" required />
      ...
      <Dialog open={isDeleteOpen} onClose={() => setIsDeleteOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <Dialog.Panel className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <p className="mt-4">هل أنت متأكد من حذف هذا الخبر؟</p>
