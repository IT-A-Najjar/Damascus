...
export default function Projects() {
  ...
  const getDatatype = async () => {
    try {
      const result = await fetchData(`/projects-status`);
      // console.log(result);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    if (!formData.Image) {
      toast("الرجاء اختيار صورة للمشروع.");
      return;
    }
    ...
    formDataToSend.append("ExecutingAuthority", formData.ExecutingAuthority);
    formDataToSend.append("LinkExecutingAuthority", formData.LinkExecutingAuthority);

    try {
      await addData("/projects", formDataToSend);
      setIsOpen(false);
      toast.success('تم اضافة المشروع');
      setLoading(false);
        ...
        <table className="bg-white shadow-md rounded-lg overflow-hidden">
          <thead className="bg-[#42790c] text-white font-bold">
            <tr>
              ...
              <th className="px-4 py-2">حالة المشروع</th>
              <th className="px-4 py-2">الاسم</th>
              <th className="px-4 py-2">الصورة</th>
              <th className="px-4 py-2">الشركة المنفذة</th>
              <th className="px-4 py-2">رابط الشركة المنفذء</th>
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={index} className="border-b hover:bg-gray-100 transition-all">
                ...
                <td className="px-4 py-2 text-center">{item.ProjectStatus}</td>
                ...
                <td className="px-4 py-2 text-center">
                  {item.ExecutingAuthority && item.ExecutingAuthority.length > 30 ? item.ExecutingAuthority.substring(0, 30) + "..." : item.ExecutingAuthority || "—"}
                </td>
                <td className="px-4 py-2 text-center">
                  <a href={item.LinkExecutingAuthority} >انتقل</a>
                </td>
      ...
      <Dialog open={isOpen} onClose={() => setIsOpen(false)} className="relative z-50">
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <Dialog.Panel className="bg-white p-6 rounded-lg shadow-lg w-96">
            <Dialog.Title className="text-lg font-bold">إضافة مشروع جديد</Dialog.Title>
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
              <input type="text" name="Name" value={formData.Name} onChange={handleInputChange} placeholder="اسم المشروع" className="border p-2 rounded" required />
              <select
                name="ProjectStatus"
              ...
              >
                ...
                {datatype.map((type) => (
                  <option key={type.id} value={type.id}>{type.ProjectStatus}</option>
                ))}
              </select>
              <input type="file" onChange={handleFileChange} className="border p-2 rounded" required />
              <input type="text" name="ExecutingAuthority" value={formData.ExecutingAuthority} onChange={handleInputChange} placeholder="الجهة المنفذة" className="border p-2 rounded" required />
              <input type="text" name="LinkExecutingAuthority" value={formData.LinkExecutingAuthority} onChange={handleInputChange} placeholder="رابط تواصل الجهة" className="border p-2 rounded" required />
