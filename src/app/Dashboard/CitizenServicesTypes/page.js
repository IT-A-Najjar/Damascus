...
export default function CitizenServicesTypes() {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteData(`/citizen-service-types/${selectedItem.id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    formDataToSend.append("CitizenServiceType", formData.CitizenServiceType);
    formDataToSend.append("CitizenServicesID", formData.CitizenServicesID);
    if (formData.PDFLink) {
      formDataToSend.append("PDFLink", formData.PDFLink);
    }

    try {
      await addData("/citizen-service-types", formDataToSend);
      setIsOpen(false);
      setLoading(false);
          ...
          <tbody>
            {data.map((item, index) => (
              <tr key={index} className="border-b hover:bg-gray-100 transition-all">
                ...
                <td className="px-4 py-2 text-center">{item.citizen_service.CitizenService}</td>
                <td className="px-4 py-2 text-center">
                  {item.CitizenServiceType && item.CitizenServiceType.length > 20 ? item.CitizenServiceType.substring(0, 20) + "..." : item.CitizenServiceType || "—"}
                </td>
                <td className="px-4 py-2 text-center">
        ...
        <div className="fixed inset-0 flex items-center justify-center">
          <Dialog.Panel className="bg-white p-6 rounded-lg shadow-lg w-96">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 mt-4">
              <input type="text" name="CitizenServiceType" value={formData.CitizenServiceType} onChange={handleInputChange} placeholder="عنوان نوع الخدمة" className="border p-2 rounded" required />
              <select
                name="CitizenServicesID"
                value={formData.CitizenServicesID}
              ...
              >
                ...
                {datatype.map((type) => (
                  <option key={type.id} value={type.id}>{type.CitizenService}</option>
                ))}
              </select>
